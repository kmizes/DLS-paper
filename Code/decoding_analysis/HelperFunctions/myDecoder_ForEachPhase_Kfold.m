function [fitStats, y_pred_all, y_pred_all_phase] = myDecoder_ForEachPhase_Kfold(X, y, y_phase, trainVsTest, decode_method, R2_sets, varargin)
...
% y_phase indicates phase/time of each observation within its trial. should
% be whole numbers.
...
for k = 1 : nFolds
    ...
    if isnumeric(X) % X is obs * features
       ...
       y_ph_train = cell2mat(y_phase(trainVsTest~=k));   
       y_ph_test = cell2mat(y_phase(trainVsTest==k));      
    end
    
    [y_pred, y_train_pred] = decodeFun(X_train, y_train, X_test, decode_method, trainCVfold);
    y_pred_phase = NaN(size(y_pred)); 
    for b = 1 : nphases
        y_pred_phase(y_ph_test==phases(b),:) = decodeFun(X_train(y_ph_train==phases(b),:), y_train(y_ph_train==phases(b),:), ...
                              X_test(y_ph_test==phases(b),:), decode_method, trainCVfold);
    end
    ...
    for r = 1 : length(R2_sets)
        ...
        tempStats(k,r).phases = phases(:)';
        tempStats(k,r).n_phase = arrayfun(@(b) sum(y_ph_test==b), phases(:)');

        R2_phase = NaN(1,nphases);
        mse_phase = NaN(1,nphases);
        for b = 1 : nphases
            [R2_phase(b), mse_phase(b)] = ...
                myR2(y_test(y_ph_test==phases(b),R2_sets{r}), y_pred_phase(y_ph_test==phases(b),R2_sets{r}));
        end
    ...
    if isnumeric(y)
        ...
        y_pred_all_phase(trainVsTest==k,:) = y_pred_phase;
    else
        for tr = unique(y_tr_test(:,1))'
            y_pred_all(tr,:) = num2cell(y_pred(y_tr_test(:,1)==tr,:),1);
            y_pred_all_phase(tr,:) = num2cell(y_pred_phase(y_tr_test(:,1)==tr,:),1);
        end
    end
end
...
function [y_pred, y_train_pred] = decodeFun(X_train, y_train, X_test, decode_method, trainCVfold)
    ...
    switch decode_method
        ...
        case 'SVR'
            ...
            y_pred = NaN(size(X_test,1), size(y_train,2));        
