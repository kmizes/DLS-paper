function [fitStats, y_pred_all] = myDecoder_SeqPhase_Kfold_R2wts(X, y, y_phase, trainVsTest, decode_method, R2_sets, R2_wts, varargin)
...
for k = 1 : nFolds
    ...
    for r = 1 : length(R2_sets)
        ...
        for b = 1 : length(phase_bins)-1
            [R2_phase(b), mse_phase(b)] = myR2(y_test(ph_bn==b,R2_sets{r}), y_pred(ph_bn==b,R2_sets{r}), R2_wts{r});
            [R2_phase_sh(b), mse_phase_sh(b)] = myR2(y_test(ph_bn_sh==b,R2_sets{r}), y_pred(ph_bn_sh==b,R2_sets{r}), R2_wts{r});
        end
